<template>
  <div class="h-full flex flex-col text-white">
    ...
    <!-- EHS Weekly Reports: use API with pagination -->
    <template v-else-if="activeTab === 'ehs'">
      <div class="flex-1 overflow-y-auto px-6 pb-4 custom-scrollbar">
        <!-- Loading -->
        <div v-if="ehsLoading" class="col-span-full h-64 flex flex-col items-center justify-center opacity-50">
          <div class="w-10 h-10 border-2 border-white/20 border-t-[#FFE600] rounded-full animate-spin mb-4"></div>
          <p class="text-sm">加载中...</p>
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div 
            v-for="report in ehsReports" 
            :key="report.id"
            @click="$emit('enterDetail', { ...report, type: 'ehs' })"
            class="group relative cursor-pointer rounded-2xl border border-white/10 bg-white/5 p-6 transition-all hover:bg-white/10"
          >
            <div class="flex items-center gap-3">
              <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-orange-400/10">
          ...
          <!-- Empty State -->
          <div v-if="ehsReports.length === 0" class="col-span-full h-64 flex flex-col items-center justify-center opacity-50">
            <div class="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-4">
              <FileText :size="32" />
            </div>
            <p class="text-lg font-medium">暂无EHS周报数据</p>
          </div>
        </div>
      </div>

      <!-- EHS Pagination -->
      <div v-if="ehsTotalPages > 1" class="flex items-center justify-center gap-2 px-6 pb-6 pt-2">
        <button
          @click="goToEHSPage(ehsCurrentPage - 1)"
          :disabled="ehsCurrentPage <= 1"
          class="px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-sm text-white/70 transition-all hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed"
        >
          上一页
        </button>
        ...
        <button
          v-for="page in ehsVisiblePages"
          :key="page"
          @click="goToEHSPage(page)"
          class="w-9 h-9 rounded-lg border text-sm font-bold transition-all"
          :class="page === ehsCurrentPage
            ? 'border-[#FFE600] bg-[#FFE600] text-[#260A2F]'
            : 'border-white/10 bg-white/5 text-white/70 hover:bg-white/10'"
        >
          {{ page }}
        ...
        <button
          @click="goToEHSPage(ehsCurrentPage + 1)"
          :disabled="ehsCurrentPage >= ehsTotalPages"
          class="px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-sm text-white/70 transition-all hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed"
        >
          下一页
        </button>
    ...
    <!-- Fallback -->
    <div v-else class="flex-1 overflow-y-auto px-6 pb-6 custom-scrollbar flex items-center justify-center">
      <p class="text-white/30">暂无数据</p>
    </div>
  </div>
</template>
...
// ========== EHS 周报 API 数据（复用周报列表接口） ==========
const ehsReports = ref<WeeklyReportItem[]>([]);
const ehsLoading = ref(false);
const ehsCurrentPage = ref(1);
const ehsTotal = ref(0);
const ehsPageSize = 9;

const ehsTotalPages = computed(() => Math.ceil(ehsTotal.value / ehsPageSize) || 1);

const ehsVisiblePages = computed(() => {
  const pages: number[] = [];
  const tp = ehsTotalPages.value;
  const cp = ehsCurrentPage.value;
  let start = Math.max(1, cp - 2);
  let end = Math.min(tp, cp + 2);
  if (end - start < 4) {
    if (start === 1) end = Math.min(tp, start + 4);
...
async function loadEHSReports() {
  if (!props.projectId) return;
  ehsLoading.value = true;
  try {
    const res = await fetchWeeklyReports(props.projectId, ehsCurrentPage.value, ehsPageSize);
    // 名称追加 "EHS周报"
    ehsReports.value = res.list.map(item => ({
      ...item,
      type: item.type + ' EHS周报',
    }));
    ehsTotal.value = res.total;
  } catch (e) {
    console.error('加载EHS周报列表失败:', e);
    ehsReports.value = [];
    ehsTotal.value = 0;
  } finally {
...
function goToEHSPage(page: number) {
  if (page < 1 || page > ehsTotalPages.value) return;
  ehsCurrentPage.value = page;
  loadEHSReports();
}
...
watch(() => props.projectId, () => {
  ...
  if (activeTab.value === 'ehs') loadEHSReports();
}, { immediate: true });
</script>
